...
exports.rewardsController = exports.RewardsController = void 0;
...
class RewardsController {
    async getPartners(req, res) {
        try {
            ...
            let partners;
            if (category) {
                partners = await rewardsService_1.rewardsService.getPartnersByCategory(category);
            }
            else {
                partners = await rewardsService_1.rewardsService.getActivePartners();
            }
            res.json({
                success: true,
                data: partners
    ...
    async getPartner(req, res) {
        try {
            const { id } = req.params;
            const partner = await rewardsService_1.rewardsService.getPartnerById(id);
            if (!partner) {
                res.status(404).json({
                    success: false,
    ...
    async checkEligibility(req, res) {
        try {
            ...
            const eligibility = await rewardsService_1.rewardsService.checkEligibility(userId, partner_id);
            res.json({
                success: true,
                data: eligibility
            });
    ...
    async requestQuote(req, res) {
        try {
            ...
            const lead = await rewardsService_1.rewardsService.requestQuote(quoteRequest);
            res.status(201).json({
                success: true,
                data: lead,
    ...
    async getUserLeads(req, res) {
        try {
            ...
            const leads = await rewardsService_1.rewardsService.getUserLeads(userId);
            res.json({
                success: true,
                data: leads
            });
    ...
    async getRewardsStats(_req, res) {
        try {
            const stats = await rewardsService_1.rewardsService.getRewardsStats();
            res.json({
                success: true,
                data: stats
            });
...
exports.rewardsController = new RewardsController();
