...
class RewardsService {
    ...
    async getPartnersByCategory(category) {
      ...
      ORDER BY name
    `;
        const result = await database_1.db.query(query, [category]);
        return result;
    }
    ...
    `;
        const result = await database_1.db.query(query, [partnerId]);
        return result[0] || null;
    }
    async checkEligibility(userId, partnerId) {
      ...
      LIMIT 1
    `;
        const scoreResult = await database_1.db.query(userScoreQuery, [userId]);
    ...
    `;
        ...
        const partnersResult = await database_1.db.query(partnersQuery, queryParams);
    ...
    async requestQuote(quoteRequest) {
      ...
      LIMIT 1
    `;
        const scoreResult = await database_1.db.query(scoreQuery, [user_id]);
    ...
    `;
        ...
        const result = await database_1.db.query(insertQuery, values);
    ...
    async updateLeadStatus(leadId, status, notes) {
      ...
      WHERE id = $3
    `;
        await database_1.db.query(query, [status, notes, leadId]);
