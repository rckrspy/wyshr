...
class OCRService {
     ...
     */
    compareNames(extractedName, verifiedName) {
     ...
     */
    requiresManualReview(ocrConfidence, nameMatchScore, documentType) {
        ...
        if (documentType === 'insurance' && nameMatchScore < 0.9)
            return true;
        return false;
    }
    /**
     ...
     */
    async processVehicleDocument(documentUrl, expectedName, expectedLicensePlate) {
        try {
            ...
            const requiresReview = this.requiresManualReview(ocrResult.confidence, nameMatch, ocrResult.extractedData.documentType || 'unknown');
